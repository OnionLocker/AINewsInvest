...
## 二十一、Phase 12: 稳定性与生产化改造 [P0-P1]

**背景**：真实运行测试中发现报告生成链路脆弱、外部数据源不稳定、乱码残留、
部署方式不适合外网访问等问题。本阶段聚焦"从能跑到可用"的结构性修复。

---

### 12.1 报告生成异步化 [P0]

**目标**：用户点击"生成"不再傻等几十秒，失败有明确分类提示。

- [x] **12.1.1** `app.py` 新增异步 Job 模型与管理
  - 内存 Job 存储 (dict)，字段: id, status, market, progress, result, error, created_at, finished_at
  - status 枚举: pending / running / success / failed

- [x] **12.1.2** `app.py` 重构 `/api/report/generate` → 异步 Job
  - POST 立即返回 job_id
  - 后台线程执行报告生成
  - 新增 `GET /api/report/job/<job_id>` 查询 job 状态/进度/结果

- [x] **12.1.3** `templates/dashboard.html` 前端改为轮询 Job 状态
  - 点击生成 → 获得 job_id → 每 2s 轮询
  - 显示进度百分比和当前步骤
  - 失败时显示分类错误信息（数据源超时/连接失败/空结果等）
  - 成功后自动刷新报告

- [x] **12.1.4** 错误分类体系
  - DataSourceTimeout / DataSourceConnectionError / DataSourceEmpty
  - LLMError / ParameterError / AuthError
  - 前端按分类显示不同提示文案
...
**目标**：行情/新闻/K线外部依赖不再一挂就拖垮系统。

- [x] **12.2.1** 新建 `utils/data_fetcher.py` 统一数据访问层
  - 封装: timeout / retry / 熔断 / fallback / 缓存
  - 统一错误分类与日志
  - 所有外部调用统一经过此层

- [x] **12.2.2** `data/market_data.py` 重构
  - 所有行情获取加硬超时 (10s)
  - 单只失败不阻塞批量
  - akshare 调用加 retry + fallback

- [x] **12.2.3** `analysis/technical.py` 重构为并发执行
  - 用 ThreadPoolExecutor 并发分析多只股票
  - 单只超时 15s，失败跳过
  - 汇总部分成功结果

- [x] **12.2.4** `analysis/news_fetcher.py` 修复与降级
  - 修复现有正则/接口异常
  - 新闻获取失败不阻塞主报告
  - 加超时和 fallback（返回空列表）

- [x] **12.2.5** `analysis/report_generator.py` 重构
  - 使用并发执行每只股票的分析
  - 逐只汇报进度给 Job
  - 部分失败仍输出报告（标注哪些失败）
...
### 12.3 Symbols 构建稳定化 [P1]

**目标**：搜索功能稳定可用，不依赖一次性实时抓取的运气。

- [x] **12.3.1** `scripts/build_symbol_db.py` 重构
  - 分市场独立构建，单市场失败不影响整体
  - 失败时保留上次成功的 symbols.json
  - 支持增量更新模式（--incremental）

- [x] **12.3.2** 启动时 symbols 容错
  - symbols.json 不存在或为空时提供内置 fallback 列表
  - 搜索 API 无 symbols 时返回友好提示而非崩溃
...
- [x] **12.4.1** `app.py` 全量修复乱码
  - 文件内所有 mojibake 中文替换为正确文本
  - 涵盖: 注释、flash 消息、日志文本、路由文档字符串

- [x] **12.4.2** `templates/deep_analysis.html` 全量修复乱码
  - HTML 文案、JS 字符串中的 mojibake 全部替换

- [x] **12.4.3** `templates/settings.html` 全量修复乱码

- [x] **12.4.4** 其他文件乱码扫描修复
  - scripts/scheduler.py
  - 确认所有 .py / .html 文件无残留乱码
...
**目标**：外网访问稳定，不再依赖 Flask debug server。

- [x] **12.5.1** `gunicorn_conf.py` 完善
  - worker 数量、超时、日志格式配置

- [x] **12.5.2** 新建 `deploy/alpha-vault.service` systemd 服务文件

- [x] **12.5.3** 新建 `deploy/nginx.conf` 反向代理配置
  - 静态文件直出
  - WebSocket/SSE 代理
  - 超时与 buffer 配置

- [x] **12.5.4** 新建 `deploy/README-deploy.md` 部署文档
  - 完整部署步骤
  - 启动/重启/日志命令
  - 环境变量说明

- [x] **12.5.5** 健康检查增强
  - `/api/health` 增加数据库连通性检查
  - 增加 readiness 检查端点
...
- [x] **12.6.1** 报告生成链路增加阶段性耗时日志
  - 每个阶段: 股票池/K线/新闻/基本面/LLM/写库
  - 带 job_id, market, ticker, 阶段名, 耗时, 错误类别

- [x] **12.6.2** `utils/logger.py` 增强日志格式
  - 结构化字段: timestamp, level, module, job_id, ticker, market, duration_ms
  - 区分 info / warning / error

- [x] **12.6.3** 请求级别日志
  - Flask before_request / after_request 记录请求耗时
  - 慢请求 (>5s) 自动 warning
...
### 12.1 报告生成异步化 [P0] ✅
- [x] 12.1.1 Job 模型与管理 — `ReportJob` SQLAlchemy 模型
- [x] 12.1.2 异步生成 API — 后台线程 + job_id 轮询
- [x] 12.1.3 前端轮询 UI — dashboard 进度条 + 状态轮询
- [x] 12.1.4 错误分类体系 — `classify_error()` + `ERROR_MESSAGES`

### 12.2 外部数据源重构 [P0] ✅
- [x] 12.2.1 统一数据访问层 `utils/data_fetcher.py` — timeout/retry/熔断/缓存
- [x] 12.2.2 行情获取加固 — market_data.py 全面用 safe_fetch 装饰器
- [x] 12.2.3 技术分析并发化 — report_generator.py ThreadPoolExecutor(4)
- [x] 12.2.4 新闻抓取修复与降级 — news_fetcher.py safe_fetch + 降级空列表
- [x] 12.2.5 报告生成器并发+进度 — 并发分析 + 单任务超时 + 总超时

### 12.3 Symbols 构建 [P1] ✅
- [x] 12.3.1 build_symbol_db 重构 — 增量更新 + 备份旧文件 + CLI 参数
- [x] 12.3.2 启动 fallback — 内置 8 只核心标的 fallback

### 12.4 编码修复 [P0] ✅
- [x] 12.4.1 app.py 全量乱码修复
- [x] 12.4.2 deep_analysis.html 乱码修复
- [x] 12.4.3 settings.html 乱码修复
- [x] 12.4.4 其他文件乱码扫描 — news_fetcher.py 完整重写
- [x] 12.4.5 统一错误文案 — data_fetcher.py ERROR_MESSAGES

### 12.5 生产部署 [P1] ✅
- [x] 12.5.1 gunicorn 配置 — `gunicorn.conf.py`
- [x] 12.5.2 systemd + nginx + 部署文档 — `deploy/` 目录
- [x] 12.5.3 健康检查增强 — DB + 符号库检查 + 503 degraded

### 12.6 可观测性 [P1] ✅
- [x] 12.6.1 阶段耗时日志 — `log_phase()` context manager
- [x] 12.6.2 日志格式增强 — `StructuredFormatter` + request_id
- [x] 12.6.3 请求级别日志 — `init_request_logging()` Flask 中间件
...
P0 先行（必须修）:
  12.4 编码修复 → 不依赖其他，立即做
  12.2 数据源重构 → 12.1 报告异步化的基础
  12.1 报告异步化 → 依赖 12.2 数据源层

P1 跟进:
  12.3 Symbols → 独立可做
  12.5 生产部署 → 独立可做
  12.6 可观测性 → 与 12.1/12.2 协同
```

实施顺序: 12.4 → 12.2 → 12.1 → 12.6 → 12.3 → 12.5
