...
function WinRateRing({ rate, size = 120 }) {
  ...
  const color = rate >= 60 ? "#16A34A" : rate >= 45 ? "#D97706" : "#DC2626";
  return (
    <div className="relative" style={{ width: size, height: size }}>
      <svg width={size} height={size} className="-rotate-90">
        <circle cx={size/2} cy={size/2} r={r} fill="none" stroke="#E7E2DE" strokeWidth={10} />
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        <span className="text-[10px] text-secondary">{"\u80DC\u7387"}</span>
      </div>
    </div>
  );
}
    ...
    <div className="rounded-2xl border border-border bg-white p-6 shadow-md">
      ...
      <div className="flex items-center gap-8 mb-6">
        ...
        <div className="grid grid-cols-2 gap-3 flex-1">
          ...
          <StatBox icon={TrendingUp} label={"\u5B8C\u5168\u80DC"} value={d.wins || 0} color="#16A34A" />
          {(d.partial_wins || 0) > 0 && (
            <StatBox icon={Target} label={"\u90E8\u5206\u6B62\u76C8"} value={d.partial_wins} color="#2563EB" />
          )}
          {(d.trailing_stops || 0) > 0 && (
            <StatBox icon={TrendingUp} label={"\u8FFD\u8E2A\u6B62\u76C8"} value={d.trailing_stops} color="#2563EB" />
          )}
          <StatBox icon={TrendingDown} label={"\u6B62\u635F"} value={d.losses || 0} color="#DC2626" />
          <StatBox icon={Clock} label={"\u8D85\u65F6"} value={d.timeouts || 0} color="#8B7E74" />
        </div>
      </div>

      <div className="grid grid-cols-3 gap-3">
        <div className="rounded-2xl border border-border bg-white p-3 text-center">
          <p className="text-[11px] text-secondary mb-1">{"\u5E73\u5747\u6536\u76CA"}</p>
          <p className="text-sm font-bold" style={{ color: (d.avg_return_pct || 0) >= 0 ? "#16A34A" : "#DC2626" }}>
            {(d.avg_return_pct || 0) >= 0 ? "+" : ""}{(d.avg_return_pct || 0).toFixed(2)}%
          </p>
        </div>
        <div className="rounded-2xl border border-border bg-white p-3 text-center">
          <p className="text-[11px] text-secondary mb-1">{"\u5E73\u5747\u76C8\u5229"}</p>
          <p className="text-sm font-bold text-[#16A34A]">
            +{(d.avg_win_return_pct || 0).toFixed(2)}%
          </p>
        </div>
        <div className="rounded-2xl border border-border bg-white p-3 text-center">
          <p className="text-[11px] text-secondary mb-1">{"\u5E73\u5747\u4E8F\u635F"}</p>
      ...
      <div className="mt-3 grid grid-cols-3 gap-3">
        <div className="rounded-2xl border border-border bg-white p-3 text-center">
          <p className="text-[11px] text-secondary mb-1">Profit Factor</p>
          <p className="text-sm font-bold" style={{ color: (d.profit_factor || 0) >= 1.5 ? "#16A34A" : (d.profit_factor || 0) >= 1.0 ? "#D97706" : "#DC2626" }}>
            {(d.profit_factor || 0).toFixed(2)}
          </p>
        </div>
        <div className="rounded-2xl border border-border bg-white p-3 text-center">
          <p className="text-[11px] text-secondary mb-1">{"\u6700\u4F73\u4EA4\u6613"}</p>
          <p className="text-sm font-bold text-[#16A34A]">
            +{(d.best_trade || 0).toFixed(2)}%
          </p>
        </div>
        <div className="rounded-2xl border border-border bg-white p-3 text-center">
          <p className="text-[11px] text-secondary mb-1">{"\u6700\u5DEE\u4EA4\u6613"}</p>
          <p className="text-sm font-bold text-[#DC2626]">
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        <div className="rounded-2xl border border-border bg-white p-3">
          <p className="text-[11px] text-secondary mb-1">{"\u8FD17\u5929\u80DC\u7387"}</p>
          <p className="text-lg font-bold text-primary">
            {d7.total_evaluated > 0 ? `${d7.win_rate}%` : "--"}
            <span className="ml-1 text-[11px] text-secondary">({d7.wins || 0}W / {d7.total_evaluated || 0}T)</span>
          </p>
        </div>
        <div className="rounded-2xl border border-border bg-white p-3">
          <p className="text-[11px] text-secondary mb-1">{"\u8FD130\u5929\u80DC\u7387"}</p>
          <p className="text-lg font-bold text-primary">
            {d30.total_evaluated > 0 ? `${d30.win_rate}%` : "--"}
            <span className="ml-1 text-[11px] text-secondary">({d30.wins || 0}W / {d30.total_evaluated || 0}T)</span>
          </p>
        </div>
      </div>
...
function DimensionPanel({ title, items }) {
  ...
  return (
    <div>
      <h4 className="mb-2 text-xs font-semibold text-secondary">{title}</h4>
      <div className="flex flex-wrap gap-3">
        {items.map((it) => {
          const wr = it.win_rate || 0;
          const wrColor = wr >= 60 ? "#16A34A" : wr >= 45 ? "#D97706" : "#DC2626";
          return (
            <div key={it.key} className="rounded-2xl border border-border bg-white px-4 py-3 min-w-[180px] flex-1">
              <div className="flex items-center gap-2 mb-1">
                <span className="rounded bg-surface-3 px-1.5 py-0.5 text-[11px] font-medium text-secondary">
              ...
              <p className="text-[11px] text-secondary">
                ...
                <span className="ml-2" style={{ color: (it.avg_return || 0) >= 0 ? "#16A34A" : "#DC2626" }}>
...
function TrendChart({ data }) {
  ...
  return (
    <div className="rounded-2xl border border-border bg-white p-6">
      <h3 className="mb-4 text-sm font-semibold text-secondary">{"\u80DC\u7387\u8D8B\u52BF (\u8FD130\u5929)"}</h3>
      <div className="flex items-end gap-1" style={{ height: 120 }}>
        {data.slice().reverse().map((d, i) => {
          const h = Math.max(4, (d.total / maxTotal) * 100);
          const wr = d.total > 0 ? d.wins / d.total : 0;
          const color = wr >= 0.6 ? '#16A34A' : wr >= 0.4 ? '#D97706' : '#DC2626';
          return (
  ...
  return (
    <button onClick={onClick}
      className={`rounded-lg px-3 py-1.5 text-sm transition-colors ${
        active ? "bg-surface-3 text-primary font-medium" : "text-secondary hover:text-primary hover:bg-surface-2"
      }`}>
      {label}
    </button>
  );
...
function DetailsTable({ items }) {
  ...
  if (!items || items.length === 0) {
    return <p className="text-sm text-secondary text-center py-8">{"\u6682\u65E0\u8BC4\u4F30\u8BB0\u5F55"}</p>;
  }
  return (
    <>
      {/* Desktop table */}
      <div className="hidden lg:block">
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            ...
            <tbody>
              {sorted.map((it, i) => {
                const outcomeMap = { win: { text: "\u80DC", color: "#16A34A", bg: "#16A34A" }, trailing_stop: { text: "\u8FFD\u8E2A\u6B62\u76C8", color: "#2563EB", bg: "#2563EB" }, partial_win: { text: "\u90E8\u5206\u6B62\u76C8", color: "#2563EB", bg: "#2563EB" }, loss: { text: "\u8D25", color: "#DC2626", bg: "#DC2626" }, timeout: { text: "\u8D85\u65F6", color: "#8B7E74", bg: "#8B7E74" } };
                const o = outcomeMap[it.outcome] || outcomeMap.timeout;
                const mktLabel = it.market === "us_stock" ? "\u7F8E\u80A1" : "\u6E2F\u80A1";
                const isShort = it.direction === "short";
                return (
                  <tr key={it.id || i} className="border-b border-border hover:bg-surface-2">
                    <td className="py-2 text-primary">{it.run_date}</td>
                    <td className="py-2 font-medium text-primary">{it.ticker} <span className="text-secondary">{it.name}</span></td>
                    <td className="py-2 text-secondary">{mktLabel}</td>
                    <td className="py-2 text-center">
                      <span className={`rounded px-1.5 py-0.5 text-[10px] font-bold ${isShort ? "bg-[#9333EA]/12 text-[#9333EA]" : "bg-[#16A34A]/12 text-[#16A34A]"}`}>
                        {isShort ? "SHORT" : "LONG"}
                      </span>
                    </td>
                    <td className="py-2 text-center">
                      {it.strategy && (
                        <span className="rounded bg-surface-3 px-1.5 py-0.5 text-[10px] text-secondary">
                          {it.strategy === "swing" ? "\u6CE2\u6BB5" : it.strategy === "short_term" ? "\u77ED\u7EBF" : it.strategy}
                        </span>
                      )}
                    </td>
                    <td className="py-2 text-right text-primary">{it.entry_price?.toFixed(2)}</td>
                    <td className="py-2 text-right text-[#16A34A]">{it.take_profit?.toFixed(2)}</td>
                    <td className="py-2 text-right text-[#DC2626]">{it.stop_loss?.toFixed(2)}</td>
                    <td className="py-2 text-right text-primary">{it.exit_price?.toFixed(2) || "--"}</td>
                    <td className="py-2 text-right font-medium" style={{ color: (it.return_pct || 0) >= 0 ? "#16A34A" : "#DC2626" }}>
                      {it.return_pct != null ? `${it.return_pct >= 0 ? "+" : ""}${it.return_pct.toFixed(2)}%` : "--"}
                    </td>
                    <td className="py-2 text-center">
                      {it.tech_score != null ? (
                        <span className="text-[11px] tabular-nums" style={{ color: it.tech_score >= 60 ? "#16A34A" : it.tech_score >= 40 ? "#e2e8f0" : "#DC2626" }}>
                    ...
                    <td className="py-2 text-center">
                      {it.news_score != null ? (
                        <span className="text-[11px] tabular-nums" style={{ color: it.news_score >= 60 ? "#16A34A" : it.news_score >= 40 ? "#e2e8f0" : "#DC2626" }}>
                    ...
                    <td className="py-2 text-center">
                      {it.fundamental_score != null ? (
                        <span className="text-[11px] tabular-nums" style={{ color: it.fundamental_score >= 60 ? "#16A34A" : it.fundamental_score >= 40 ? "#e2e8f0" : "#DC2626" }}>
                    ...
                    <td className="py-2 text-center">
                      {(it.confidence || it.combined_score) != null ? (
                        <span className="text-[11px] font-semibold tabular-nums" style={{ color: (it.confidence || it.combined_score) >= 65 ? "#16A34A" : (it.confidence || it.combined_score) >= 45 ? "#D97706" : "#DC2626" }}>
                    ...
                    <td className="py-2 text-center">
                      <span className="rounded px-2 py-0.5 text-[11px] font-semibold"
                        style={{ color: o.color, background: o.bg + "08" }}>{o.text}</span>
                    </td>
                  </tr>
                );
              })}
      ...
      <div className="lg:hidden space-y-2">
        {sorted.map((it, i) => {
          ...
          const scoreColor = (v) => v >= 60 ? "#16A34A" : v >= 40 ? "#e2e8f0" : "#DC2626";
          const confValue = it.confidence || it.combined_score;
          const confColor = confValue >= 65 ? "#16A34A" : confValue >= 45 ? "#D97706" : "#DC2626";

          return (
            <div
              key={cardKey}
              className="rounded-xl border border-border bg-surface-0 p-3 cursor-pointer active:bg-surface-2 transition-colors"
              onClick={() => setExpandedId(isExpanded ? null : cardKey)}
            >
              {/* Top row: date + outcome */}
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-[11px] text-secondary">{it.run_date}</span>
                <span className="rounded px-2 py-0.5 text-[11px] font-semibold"
                  style={{ color: o.color, background: o.bg + "08" }}>{o.text}</span>
              </div>
              ...
              <div className="flex items-center gap-2 mb-2">
                <span className="text-sm font-semibold text-primary">{it.ticker}</span>
                <span className="text-xs text-secondary truncate">{it.name}</span>
                <span className={`ml-auto shrink-0 rounded px-1.5 py-0.5 text-[10px] font-bold ${isShort ? "bg-[#9333EA]/12 text-[#9333EA]" : "bg-[#16A34A]/12 text-[#16A34A]"}`}>
                  {isShort ? "SHORT" : "LONG"}
                </span>
              </div>
              ...
              <div className="flex items-center gap-1.5 text-xs">
                <span className="text-secondary">{it.entry_price?.toFixed(2)}</span>
                <span className="text-tertiary">{"\u2192"}</span>
                <span className="text-primary">{it.exit_price?.toFixed(2) || "--"}</span>
                <span className="ml-auto font-medium" style={{ color: (it.return_pct || 0) >= 0 ? "#16A34A" : "#DC2626" }}>
                  {it.return_pct != null ? `${it.return_pct >= 0 ? "+" : ""}${it.return_pct.toFixed(2)}%` : "--"}
                </span>
              </div>
                ...
                <div className="mt-3 border-t border-border pt-3 space-y-2">
                  ...
                  <div className="flex items-center gap-3 text-xs">
                    ...
                    <span className="text-secondary">
                      SL <span className="text-[#DC2626]">{it.stop_loss?.toFixed(2) || "--"}</span>
                    </span>
                    <span className="text-secondary">
                      TP <span className="text-[#16A34A]">{it.take_profit?.toFixed(2) || "--"}</span>
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    <div className="rounded-lg bg-surface-2 px-2 py-1.5 text-center">
                      <p className="text-[10px] text-secondary">{"\u6280\u672F"}</p>
                      {it.tech_score != null ? (
                    ...
                    <div className="rounded-lg bg-surface-2 px-2 py-1.5 text-center">
                      <p className="text-[10px] text-secondary">{"\u7F6E\u4FE1"}</p>
                      {confValue != null ? (
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-light text-primary">{"\u7CFB\u7EDF\u80DC\u7387\u7EDF\u8BA1"}</h1>
          <p className="text-sm text-secondary">{"\u8FBE\u5230TP1\u5373\u4E3A\u80DC\u51FA\uff0c\u89E6\u53CA\u6B62\u635F\u5373\u4E3A\u5931\u8D25\uff0c\u8D85\u8FC7\u6301\u4ED3\u5929\u6570\u4E3A\u8D85\u65F6"}</p>
        </div>
        <button
          onClick={handleEvaluate}
          disabled={evaluating}
          className="flex items-center gap-1.5 rounded-lg bg-brand px-4 py-2 text-sm font-medium text-white shadow-lg shadow-brand/15 transition-colors hover:bg-[#A04E30] disabled:opacity-50"
        >
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <MarketCard title={"\u7F8E\u80A1\u80DC\u7387"} data={summary?.us_stock} color="#2563EB" />
        <MarketCard title={"\u6E2F\u80A1\u80DC\u7387"} data={summary?.hk_stock} color="#D97706" />
      </div>
      ...
      {summary && (summary.us_stock?.by_strategy || summary.hk_stock?.by_strategy) && (
        <div className="rounded-2xl border border-border bg-white p-6 space-y-5">
          <h3 className="text-sm font-semibold text-primary">{"\u7EF4\u5EA6\u5206\u6790"}</h3>
          <DimensionPanel
            title={"\u6309\u7B56\u7565"}
            items={[
      ...
      <div className="rounded-2xl border border-border bg-white p-5 shadow-md">
        <h2 className="mb-4 text-sm font-semibold text-primary">{"\u8BC4\u4F30\u8BB0\u5F55\u660E\u7EC6"}</h2>
